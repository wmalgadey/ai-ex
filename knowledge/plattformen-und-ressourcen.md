# Plattformen & Ressourcen

Communities, Kurse, Forks, Links.

## Artikel

- [Vom Agenten-Team zum KI-Betriebssystem](felix-schlenther-ki-betriebssystem.md) — AI FIRST: 25 spezialisierte Agenten auf einen universellen umgestellt
- [KI als Betriebssystem — Die 5 Bausteine](felix-schlenther-ki-betriebssystem-2.md) — Skills, Kontext, Governance, Agents, Feedback; Agent bewusst "dumm" halten; Shift: von Arbeit tun zu System bauen
- [ClawClones — The OpenClaw Ecosystem](clawclones.md) — Tracker für OpenClaw-Forks mit Community Pulse, Stars, Besonderheiten; Stand März 2026
- [Build Your Own OpenClaw](build-your-own-openclaw.md) — 18-stufiges Tutorial zum Selbstbauen einer OpenClaw-Infrastruktur, 4 Phasen von Chat-Loop bis Production
- [Claw3D](claw3d.md) — 3D-Engine auf OpenClaw-Basis, 581 Stars
- [Anthropic Courses](anthropic-courses.md) — Übersicht aller offiziellen Anthropic-Kurse auf skilljar.com
- [NVIDIA DLI — Kostenlose KI-Kurse](nvidia-dli-kurse.md) — 9 kostenlose Self-Paced-Kurse, Fokus auf Infrastruktur und Hardware-Schicht statt Tool-Nutzung
- [Microsoft Agents League](microsoft-agents-league.md) — 2-Wochen AI Developer Challenge von Microsoft (Feb 2026), Starter Kits verfügbar
- [Scrum Sage Zen Edition](scrum-gpt.md) — Custom GPT als Scrum-Coach mit Zen-Perspektive
- [Links](links.md) — Lose Link-Sammlung (entire.io, fallow.tools Codebase Intelligence, flipbook.page Generative Visual Internet)
- [Claude Managed Agents](claude-managed-agents.md) — Anthropic-verwaltete Agent-Infrastruktur: Sandboxing, Long-running Sessions, Multi-Agent; TODO: Onboarding abschließen
